...
begin
  try
    ...
    begin
      ...
      begin
        MessageDlg('CEP não encontrado.', TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0); 
        Exit;
      end
      else
      begin
...
begin
  try
    ...
    begin
      MessageDlg('Necessário prencher o CEP para carregar os dados.', TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0);
      Exit;
    end;

    if (SomenteNumero(edtCEP.Text).Length <> 8) then
    begin
      MessageDlg('CEP inválido.', TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0);
      Exit;
    end;
...
begin
  try
    ...
    begin
      ...
      //limpa campos dos edit, ja está na listview
  ...
  try
    ...
    begin
      ...
      begin
        if (cbTipoPessoa.ItemIndex = 1) then
        begin
          MessageDlg('CNPJ já cadastrado no sistema.', TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0);
          Exit;
        end
        else
        begin
          MessageDlg('CPF já cadastrado no sistema.', TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0);
          Exit;
        end;
      end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                TEdit(pForm.Components[vContador]).TextSettings.FontColor := TAlphaColors.Red;
                TEdit(pForm.Components[vContador]).TextPrompt := 'Informe a Inscrição Estadual.';
              end
              else
              begin
                TEdit(pForm.Components[vContador]).TextSettings.FontColor := TAlphaColors.Red;
